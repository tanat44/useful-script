...
void Gimbal::configH(int16_t min, int16_t max, int16_t zeroRange, int16_t offset) {
  h->setMinMax(min, max);
  h->setZeroRange(zeroRange);
  h->setCenterOffset(offset);
}

void Gimbal::configV(int16_t min, int16_t max, int16_t zeroRange, int16_t offset) {
  v->setMinMax(min, max);

...
const uint8_t motor1Pin2 = 25;
const uint8_t motor2Pin1 = 32;
const uint8_t motor2Pin2 = 12;
...
void dumpGamepad(ControllerPtr ctl) {
  ...
  uint8_t pwmLeft2 = map(pwmLeft, 0, 255, 0, maxPwm);
  uint8_t pwmRight2 = map(pwmRight, 0, 255, 0, maxPwm);

  Serial.printf("s=%d\ta=%d\t[L=%d, R=%d]\n", steer, accel, pwmLeft2, pwmRight2);

  if (!reverseMotor1) {
    if (accel > 0) {
      analogWrite(motor1Pin1, pwmLeft2);
      analogWrite(motor1Pin2, 0);
    } else {
      analogWrite(motor1Pin1, 0);
      analogWrite(motor1Pin2, pwmLeft2);
    }
  } else {
    if (accel < 0) {
      analogWrite(motor1Pin1, pwmLeft2);
  ...
  if (!reverseMotor2) {
    if (accel > 0) {
      analogWrite(motor2Pin1, pwmRight2);
      analogWrite(motor2Pin2, 0);
    } else {
      analogWrite(motor2Pin1, 0);
      analogWrite(motor2Pin2, pwmRight2);
    }
  } else {
    if (accel < 0) {
      analogWrite(motor2Pin1, pwmRight2);

...
fn byte_vector<T>(vec: Vec<T>) -> Vec<u8> {
    let byte_length = vec.len() * mem::size_of::<T>();
    let byte_capacity = vec.capacity() * mem::size_of::<T>();
    let alloc = vec.into_boxed_slice();
    let ptr = Box::<[T]>::into_raw(alloc) as *mut u8;
...
fn to_padded_byte(vertices: Vec<Vertex>, indices: Vec<u16>) -> Vec<u8> {
    let mut v1 = byte_vector(vertices);
    let mut v2 = byte_vector(indices);
    v1.append(&mut v2);
    while v1.len() % 4 != 0 {
        v1.push(0); // pad to multiple of four bytes
    }
    v1
}
...
fn export(output: Output) {
    ...
    let buffer_length = (triangle_vertices.len() * mem::size_of::<Vertex>()
        + indices.len() * mem::size_of::<u16>()) as u32;
    ...
    let buffer_view = json::buffer::View {
        ...
        byte_length: (triangle_vertices.len() * mem::size_of::<Vertex>()) as u32,
        byte_offset: None,
    ...
    let buffer_view2 = json::buffer::View {
        buffer: json::Index::new(0),
        byte_length: (indices.len() * mem::size_of::<u16>()) as u32,
        byte_offset: Some((triangle_vertices.len() * mem::size_of::<Vertex>()) as u32),
    ...
    let root = json::Root {
        accessors: vec![positions, colors, indices_acc],
        buffers: vec![buffer],
        buffer_views: vec![buffer_view, buffer_view2],
        meshes: vec![mesh],
        nodes: vec![node],
    ...
    match output {
        Output::Standard => {
            ...
            let mut bin = to_padded_byte(triangle_vertices, indices);
